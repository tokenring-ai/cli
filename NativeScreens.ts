...
function fitLine(text: string, width: number): string {
  ...
  if (width <= 1) return "…";
  return `${Array.from(text).slice(0, Math.max(0, width - 1)).join("")}…`;
}
...
function buildSelectionEntries(app: TokenRingApp): SelectionEntry[] {
  ...
  if (webHostService) {
    ...
    if (entries.length > 0) {
      categories.set("Web Application", entries.sort((left, right) => left.label.localeCompare(right.label)));
    }
  }
  ...
  for (const [type, config] of agentManager.getAgentConfigEntries()) {
    ...
    const enabledTools = ((config as any).chat as z.input<typeof ChatAgentConfigSchema> | undefined)?.enabledTools ?? [];
  ...
  if (workflowService) {
    ...
    if (workflows.length > 0) {
      categories.set("Workflows", workflows
        .map(({key, workflow}) => ({
          type: "option" as const,
...
function renderSelectionScreen(
  ...
  const flattenedLines = entries.map((entry, lineIndex) => {
    if (entry.type === "heading") {
      return {text: entry.label, tone: "heading" as const} satisfies ScreenLine;
    }
    ...
    optionCursor += 1;
    return {text: line, tone: isSelected ? "selected" as const : "normal" as const} satisfies ScreenLine;
  });

  const headerLine = width >= 70
...
function renderLoadingScreenLines(app: TokenRingApp, config: CLIConfig, renderTick: number): string[] {
  const {width, height} = getTerminalSize();
  const bannerLines = formatBanner(config, width).map(line => LOADING_COLOR(centerLine(line, width)));
  const spinnerLine = LOADING_COLOR(centerLine(`${brailleSpinner[renderTick % brailleSpinner.length]} ${getRandomItem(ridiculousMessages, renderTick / 10)}`, width));
