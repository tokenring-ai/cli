...
export default class REPLOutputFormatter {
  ...
  printHorizontalLine() {
   if (this.spinner) {
    ...
    const lineChar = '─';
    const lineWidth = process.stdout.columns ? Math.floor(process.stdout.columns * 0.8) : 60; // 80% of width or 60 chars
  ...
  // Unchanged methods:
  jobQueued(jobInfo) {
    this.systemLine(`Job [${jobInfo.name}] queued. Queue length: ${jobInfo.queueLength}`);
  }
