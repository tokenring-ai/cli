...
export default class AgentLoop {
  ...
  private deleteCurrentAgent() {
    try {
      ...
      agentManager.deleteAgent(this.agent.id, "Agent was shut down from the CLI");
    } catch (error: unknown) {
      process.stderr.write(formatLogMessages(["Error while deleting agent from CLI", error as Error]));
    }
  }

  private async consumeEvents(subscription: AsyncIterable<AgentEventState>, signal: AbortSignal): Promise<void> {
